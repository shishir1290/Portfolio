...
function GameScene({
    ...
    useEffect(() => {
        const handleMove = (e: MouseEvent) => {
            ...
            paddleX.current = Math.max(FIELD_LEFT + PADDLE_WIDTH / 2, Math.min(FIELD_RIGHT - PADDLE_WIDTH / 2, mapped));
        };
        const handleClick = () => {
...
export default function BallBounce() {
    ...
    const restart = useCallback(() => {
        setGameState({ score: 0, lives: 3, level: 0, gameOver: false, launched: false });
    }, []);
        ...
        <div className="relative w-full h-full">
            ...
            {!gameState.launched && !gameState.gameOver && (
                <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-10 px-4 py-2 bg-dark/80 backdrop-blur-md border border-primary/20 rounded-sm">
                    <p className="text-primary text-xs tracking-widest" style={{ fontFamily: "Space Mono, monospace" }}>
                        CLICK TO LAUNCH
                    </p>
                </div>
            )}
