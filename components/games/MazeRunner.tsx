    ...
    return (
        <group position={[x, 0, z]}>
            ...
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
                ...
                <meshStandardMaterial color="#00f5d4" emissive="#00f5d4" emissiveIntensity={1} transparent opacity={0.3} />
            </mesh>
            {/* Floating gem */}
...
function FPSController({
    ...
    useFrame((_, delta) => {
        ...
        if (keys.current.has("w")) newPos.addScaledVector(dir, speed);
        if (keys.current.has("s")) newPos.addScaledVector(dir, -speed);
        ...
        for (let dr = -1; dr <= 1; dr++) {
            for (let dc = -1; dc <= 1; dc++) {
                const cr = gz + dr;
                const cc = gx + dc;
                if (cr >= 0 && cr < grid.length && cc >= 0 && cc < grid[0].length && grid[cr][cc]) {
                    ...
                    const dist = new THREE.Vector2(newPos.x, newPos.z).distanceTo(closest);
                    if (dist < margin) {
                        canMove = false;
                    }
                }
...
function MazeMinimap({
    ...
    useEffect(() => {
        ...
        ctx.beginPath();
        ctx.arc(endPos[0] * cellW + cellW / 2, endPos[1] * cellH + cellH / 2, 3, 0, Math.PI * 2);
        ctx.fill();
...
export default function MazeRunner() {
    const mazeSize = 8;
    const [mazeData, setMazeData] = useState(() => generateMaze(mazeSize, mazeSize));
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            const canvas = canvasContainerRef.current?.querySelector("canvas:first-of-type") as HTMLCanvasElement;
            if (canvas) {
    ...
    return (
        <div className="relative w-full h-full" ref={canvasContainerRef}>
            <Canvas>
                ...
                <pointLight position={[1, 3, 1]} intensity={1} color="#7209b7" distance={15} />
                <pointLight position={[mazeData.end[0], 3, mazeData.end[1]]} intensity={1.5} color="#00f5d4" distance={10} />
            ...
            {/* Minimap */}
            {started && !won && (
                <MazeMinimap grid={mazeData.grid} playerPos={playerPos} endPos={mazeData.end} />
            )}
            ...
            {won && (
                <div className="absolute inset-0 flex items-center justify-center bg-dark/70 backdrop-blur-sm z-20">
                    <div className="text-center">
                        <h2 className="text-5xl font-bold gradient-text mb-2" style={{ fontFamily: "Bebas Neue, sans-serif" }}>
                            MAZE COMPLETE!
                        </h2>
                        <p className="text-3xl text-primary mb-6" style={{ fontFamily: "Bebas Neue, sans-serif" }}>
                            TIME: {formatTime(time)}
                        </p>
