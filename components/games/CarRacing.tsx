...
function WeatherParticles({ weather }: { weather: WeatherType }) {
    ...
    const [positions, velocities] = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            vel[i * 3 + 1] = -(weather === "snow" ? 0.04 + rand() * 0.03 : 0.18 + rand() * 0.12);
            vel[i * 3 + 2] = 0;
        }
        return [pos, vel];
    }, [count, weather]);
...
function Lightning({ active }: { active: boolean }) {
    ...
    return <pointLight ref={ref} position={[2, 20, -10]} color="#aaccff" distance={200} />;
}
...
function Road({ weather }: { weather: WeatherType }) {
    ...
    return (
        <group>
            ...
            {/* Sidewalks */}
            {[-1, 1].map((side, i) => (
                <mesh key={i} rotation={[-Math.PI / 2, 0, 0]} position={[side * 5.25, -0.48, 0]}>
            ...
            {[-LANE_WIDTH * 0.5, LANE_WIDTH * 0.5].map((x, i) => (
                <mesh key={i} rotation={[-Math.PI / 2, 0, 0]} position={[x, -0.49, 0]}>
                    ...
                    <meshStandardMaterial color={cfg.lineColor} opacity={0.25} transparent />
                </mesh>
            ))}
        </group>
    );
...
/* ─── Dashed Center Lines ─── */
function CenterLines({ speed, weather }: { speed: number; weather: WeatherType }) {
    ...
    return (
        <group ref={ref}>
            {Array.from({ length: 40 }).map((_, i) => (
                <mesh key={i} rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.488, -i * 4]}>
                    <planeGeometry args={[0.1, 2.2]} />
                    <meshStandardMaterial color={cfg.lineColor} opacity={0.5} transparent />
                </mesh>
            ))}
        </group>
    );
...
/* ─── Street Lamps ─── */
function StreetLamps({ speed, weather }: { speed: number; weather: WeatherType }) {
    const ref = useRef<THREE.Group>(null);
    useFrame(() => {
        if (!ref.current) return;
        ref.current.children.forEach((c) => {
        ...
        [-1, 1].forEach((side, j) => {
            lamps.push(
                <group key={`${i}-${j}`} position={[side * 4.6, 0, z]}>
                    ...
                    {/* Arm */}
                    <mesh position={[side * -0.3, 3.65, 0]} rotation={[0, 0, side * -0.3]}>
...
function Buildings({ speed, weather }: { speed: number; weather: WeatherType }) {
    const ref = useRef<THREE.Group>(null);
    ...
    const buildingData = useMemo(() => {
        ...
        for (let i = 0; i < 28; i++) {
            ...
            data.push({ h, side, xOff, w, d, z, windowRows, windowCols, seed: rand() });
        }
        return data;
    }, []);
        ...
        <group ref={ref} position={[targetX, 0.22, 7]}>
            <group ref={bodyRef}>
                ...
                <mesh position={[0, 0.28, 0]}>
                    ...
                    <meshStandardMaterial color="#cc2200" metalness={0.85} roughness={0.15} />
                </mesh>
                {/* Lower skirt */}
                <mesh position={[0, 0.1, 0]}>
                    <boxGeometry args={[1.42, 0.12, 2.7]} />
                    <meshStandardMaterial color="#880000" metalness={0.7} roughness={0.2} />
                </mesh>
                {/* Cabin */}
                <mesh position={[0, 0.56, 0.12]}>
                    <boxGeometry args={[0.98, 0.38, 1.3]} />
                    <meshStandardMaterial color="#0a0a12" metalness={0.4} roughness={0.2} transparent opacity={0.85} />
                </mesh>
                {/* Windshield */}
                <mesh position={[0, 0.55, -0.5]} rotation={[0.35, 0, 0]}>
                    <planeGeometry args={[0.9, 0.35]} />
                    <meshStandardMaterial color="#88aacc" metalness={0.1} roughness={0.05} transparent opacity={0.55} />
                </mesh>
                {/* Hood scoop */}
                <mesh position={[0, 0.49, -0.7]}>
                    <boxGeometry args={[0.4, 0.1, 0.5]} />
                    <meshStandardMaterial color="#990000" metalness={0.9} roughness={0.1} />
                </mesh>
                {/* Spoiler */}
                ...
                {[-0.52, 0.52].map((x, i) => (
                    <group key={i}>
                        <mesh position={[x, 0.28, -1.32]}>
                            ...
                            <meshStandardMaterial color="#ffffee" emissive="#ffffaa" emissiveIntensity={3} />
                        </mesh>
                        <pointLight position={[x, 0.28, -2.5]} color="#ffffcc" intensity={1.5} distance={12} />
                    </group>
                ))}
                {/* Tail lights */}
                {[-0.52, 0.52].map((x, i) => (
                ...
                {/* Exhaust glow */}
                {[-0.3, 0.3].map((x, i) => (
                    <pointLight key={i} position={[x, 0.1, 1.6]} color="#ff6600" intensity={0.6} distance={3} />
                ))}
            </group>
            {/* Wheels */}
            {wheelRefs.map((w, i) => (
...
/* ─── Obstacle Car ─── */
function ObstacleCar({ x, z, variant, speed }: { x: number; z: number; variant: number; speed: number }) {
    const w1 = useRef<THREE.Group>(null);
    const w2 = useRef<THREE.Group>(null);
    const w3 = useRef<THREE.Group>(null);
    const w4 = useRef<THREE.Group>(null);
    ...
    const darkColor = color.replace(/[0-9a-f]{2}/gi, (m) =>
        Math.max(0, parseInt(m, 16) - 0x22).toString(16).padStart(2, "0")
    );
    ...
    return (
        <group position={[x, 0.22, z]}>
            ...
            <mesh position={[0, 0.52, 0.1]}>
                ...
                <meshStandardMaterial color="#0a0a14" transparent opacity={0.8} roughness={0.2} />
            </mesh>
            {/* Tail lights */}
            {[-0.5, 0.5].map((ox, i) => (
                <mesh key={i} position={[ox, 0.26, 1.16]}>
                    <boxGeometry args={[0.2, 0.12, 0.05]} />
                    <meshStandardMaterial color="#ff3300" emissive="#ff3300" emissiveIntensity={1.8} />
                </mesh>
            ))}
            {/* Headlights */}
            {[-0.5, 0.5].map((ox, i) => (
                <mesh key={i} position={[ox, 0.26, -1.16]}>
                    <boxGeometry args={[0.2, 0.12, 0.05]} />
                    <meshStandardMaterial color="#ffeecc" emissive="#ffeecc" emissiveIntensity={2} />
                </mesh>
            ))}
            {/* Wheels */}
            {[w1, w2, w3, w4].map((w, i) => (
...
function EnvLighting({ weather }: { weather: WeatherType }) {
    ...
    return (
        <>
            <ambientLight intensity={cfg.ambientIntensity} color={weather === "snow" ? "#cce0ff" : "#ffffff"} />
            <directionalLight
                ref={sunRef}
                position={weather === "clear" ? [8, 15, -10] : [2, 8, -5]}
                intensity={weather === "clear" ? 1.4 : weather === "storm" ? 0.1 : 0.5}
                color={weather === "clear" ? "#fff5e0" : weather === "snow" ? "#ddeeff" : "#8899bb"}
                castShadow
            />
            {/* Headlight fill */}
            <pointLight position={[0, 3, 10]} intensity={0.8} color="#ffffcc" distance={15} />
            {/* Street lamp glow */}
            <pointLight position={[-4.6, 4, 0]} intensity={0.4} color="#ffcc44" distance={12} />
            <pointLight position={[4.6, 4, 0]} intensity={0.4} color="#ffcc44" distance={12} />
...
/* ─── Game Scene ─── */
function GameScene({ gameState, setGameState, weather }: {
    gameState: GameState;
    ...
    useEffect(() => {
        const handleKey = (e: KeyboardEvent) => {
            ...
            switch (e.key) {
                case "ArrowLeft": setLane((l) => Math.max(0, l - 1)); break;
                case "ArrowRight": setLane((l) => Math.min(2, l + 1)); break;
                case "ArrowUp": setSpeed((s) => Math.min(s + 0.03, 0.55)); break;
                case "ArrowDown": setSpeed((s) => Math.max(s - 0.03, 0.1)); break;
            }
        };
    ...
    useFrame((_, delta) => {
        ...
        if (spawnTimer.current > spawnInterval) {
            ...
            setObstacles((prev) => [
                ...prev,
                { id: obstacleId.current++, x: LANES[l], z: -65, lane: l, variant: Math.floor(Math.random() * 5) },
            ]);
        }

        setObstacles((prev) => {
            const next = prev.map((o) => ({ ...o, z: o.z + speed })).filter((o) => o.z < 22);
    ...
    return (
        <>
            ...
            {obstacles.map((o) => (
                <ObstacleCar key={o.id} x={o.x} z={o.z} variant={o.variant} speed={speed} />
            ))}
            {/* Ground fill */}
...
function MobileControls({ onLeft, onRight }: { onLeft: () => void; onRight: () => void }) {
    return (
        <div style={{
            ...
            padding: "0 28px", zIndex: 30, pointerEvents: "auto",
        }}>
            {[
...
/* ─── HUD ─── */
function HUD({ score, speed, weather, onWeatherChange }: {
    score: number;
    speed: number;
    weather: WeatherType;
        ...
        <div style={{
            ...
            <div style={{ textAlign: "right" }}>
                <div style={{
                    ...
                    <div style={{ color: "#ffdd88", fontSize: 11, letterSpacing: "0.15em", opacity: 0.8 }}>SCORE</div>
                    <div style={{ color: "#fff", fontSize: 26, fontWeight: 700, lineHeight: 1.1 }}>
                        {Math.floor(score / 5).toString().padStart(5, "0")}
                    </div>
                    <div style={{ color: "#88ccff", fontSize: 12, letterSpacing: "0.1em", marginTop: 4 }}>
...
function StartScreen() {
    return (
        <div style={{
            ...
            background: "rgba(0,0,0,0.72)", backdropFilter: "blur(6px)", zIndex: 20,
            fontFamily: "'Bebas Neue', Impact, sans-serif",
        }}>
            <div style={{ fontSize: 11, letterSpacing: "0.4em", color: "#888", marginBottom: 10 }}>
                TURBO DRIFT
            </div>
            <div style={{
            ...
            <div style={{
                ...
                letterSpacing: "0.2em", marginTop: 18, marginBottom: 30,
            ...
            <div style={{ display: "flex", gap: 10 }}>
                {["←", "→", "↑", "↓"].map((k) => (
                    <div key={k} style={{
                        ...
                        border: "1px solid rgba(255,255,255,0.2)",
                        borderRadius: 8, color: "#ccc", fontSize: 18,
                    }}>{k}</div>
                ))}
            </div>
            <div style={{ marginTop: 18, fontSize: 11, color: "rgba(255,255,255,0.2)", letterSpacing: "0.15em" }}>
...
function GameOverScreen({ score, onRestart }: { score: number; onRestart: () => void }) {
    return (
        <div style={{
            position: "absolute", inset: 0, display: "flex",
            flexDirection: "column", alignItems: "center", justifyContent: "center",
            background: "rgba(0,0,0,0.82)", backdropFilter: "blur(8px)", zIndex: 20,
            fontFamily: "'Bebas Neue', Impact, sans-serif",
        }}>
            <div style={{
                fontSize: 72, fontWeight: 900, color: "#ff2200",
                textShadow: "0 0 50px #ff0000", letterSpacing: "0.05em",
            }}>
                CRASH!
            </div>
            <div style={{ fontSize: 18, color: "rgba(255,255,255,0.5)", letterSpacing: "0.2em", marginTop: -8 }}>
                GAME OVER
            </div>
            <div style={{ marginTop: 20, marginBottom: 32 }}>
                <div style={{ fontSize: 13, color: "#888", letterSpacing: "0.2em", textAlign: "center" }}>FINAL SCORE</div>
                <div style={{ fontSize: 52, color: "#ffdd88", textAlign: "center", lineHeight: 1.1 }}>
                    {Math.floor(score / 5).toString().padStart(5, "0")}
                </div>
            </div>
            <button
...
export default function CarRacing() {
    const [gameState, setGameState] = useState<GameState>({ score: 0, gameOver: false, started: false });
    ...
    return (
        <div style={{ position: "relative", width: "100%", height: "100%", background: "#000", overflow: "hidden" }}>
            ...
            <Canvas
                ...
                key={resetKey}
            >
                <GameScene gameState={gameState} setGameState={setGameState} weather={weather} />
            </Canvas>
            ...
            {gameState.gameOver && <GameOverScreen score={gameState.score} onRestart={restart} />}
