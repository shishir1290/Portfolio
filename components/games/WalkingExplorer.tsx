...
function useDayNight() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (!paused) {
        timeRef.current =
          (timeRef.current + (now - last) / 1000 / DAY_CYCLE) % 1;
  ...
  useFrame((state, delta) => {
    ...
    if (lLegRef.current)
      lLegRef.current.rotation.x +=
        (legTarget - lLegRef.current.rotation.x) * 0.25;
    if (rLegRef.current)
      rLegRef.current.rotation.x +=
        (-legTarget - rLegRef.current.rotation.x) * 0.25;
...
function DynamicSkyAmbient({
  ...
  return (
    <>
      ...
      <hemisphereLight
        args={[
          "#87ceeb" as THREE.ColorRepresentation,
          "#0a1a08" as THREE.ColorRepresentation,
          0.15,
        ]}
      />
    </>
...
function Trees({
  ...
  const trunkMat = useRef(
    new THREE.MeshStandardMaterial({
      color: isSnow ? "#4a3520" : "#2c1a0e",
      roughness: 1,
    }),
  );
  const pineMat = useRef(
    new THREE.MeshStandardMaterial({
      color: isSnow ? "#c8dde8" : "#0b3d1e",
      roughness: 0.9,
    }),
  );
  const roundMat = useRef(
    new THREE.MeshStandardMaterial({
...
function Campfire({
  ...
  useFrame((state) => {
    ...
    if (embersRef.current && !interacted) {
      const pos = embersRef.current.geometry.attributes.position
    ...
    <group position={[x, 0, z]}>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <mesh
          key={i}
          position={[
            Math.cos((i / 8) * Math.PI * 2) * 0.38,
            0.06,
            Math.sin((i / 8) * Math.PI * 2) * 0.38,
          ]}
          castShadow
        >
...
function Snow({ active }: { active: boolean }) {
  ...
  useFrame((state, dt) => {
    ...
    for (let i = 0; i < count; i++) {
      pos[i * 3 + 1] -= 1.2 * dt;
      pos[i * 3] +=
        Math.sin(state.clock.elapsedTime * 0.5 + offsets.current[i]) * 0.005;
      if (pos[i * 3 + 1] < 0) {
        pos[i * 3 + 1] = 25;
        pos[i * 3] = (Math.random() - 0.5) * 80;
        pos[i * 3 + 2] = (Math.random() - 0.5) * 80;
...
function Minimap({
  ...
  weatherName: string;
  activities: Activity[];
  others: Map<string, RemotePlayer>;
}) {
  const scale = 0.5;
      ...
      <div style={{ position: "relative", width: "100%", height: "100%" }}>
        <div
          style={{
            ...
            inset: 0,
            backgroundImage:
              "repeating-linear-gradient(0deg,transparent,transparent 15px,rgba(255,255,255,0.018) 15px,rgba(255,255,255,0.018) 16px),repeating-linear-gradient(90deg,transparent,transparent 15px,rgba(255,255,255,0.018) 15px,rgba(255,255,255,0.018) 16px)",
          }}
        />
        {activities.map((a) => {
        ...
        >
          {[
            ...
            >
              <div
                style={{ width: 4, height: 4, background: c, borderRadius: 1 }}
              />
              <span
                style={{
                  fontSize: 6,
...
function OtherPlayer({ player }: { player: RemotePlayer }) {
  ...
  useFrame((_, delta) => {
    ...
    if (lLegRef.current)
      lLegRef.current.rotation.x += (swing - lLegRef.current.rotation.x) * 0.25;
    if (rLegRef.current)
      rLegRef.current.rotation.x +=
        (-swing - rLegRef.current.rotation.x) * 0.25;
  ...
  useEffect(() => {
    ...
    const onStart = (e: TouchEvent) => {
      for (const t of Array.from(e.changedTouches)) {
        ...
        if (left && mId.current === null) {
          mId.current = t.identifier;
          mC.current = { x: t.clientX, y: t.clientY };
          setMBase({ x: t.clientX, y: t.clientY });
        } else if (!left && lId.current === null) {
          lId.current = t.identifier;
          lC.current = { x: t.clientX, y: t.clientY };
          setLBase({ x: t.clientX, y: t.clientY });
        }
      }
    };
    const onMove = (e: TouchEvent) => {
      ...
      for (const t of Array.from(e.changedTouches)) {
        if (t.identifier === mId.current) {
          const dx = clamp(t.clientX - mC.current.x),
            dy = clamp(t.clientY - mC.current.y);
          if (moveJoyRef.current) {
            moveJoyRef.current.x = dx / R;
            moveJoyRef.current.y = dy / R;
          }
          setMk({ x: dx, y: dy });
        }
        if (t.identifier === lId.current) {
          const dx = clamp(t.clientX - lC.current.x),
            dy = clamp(t.clientY - lC.current.y);
          if (lookJoyRef.current) {
            lookJoyRef.current.x = dx / R;
            lookJoyRef.current.y = dy / R;
          }
          setLk({ x: dx, y: dy });
        }
      }
    ...
    const onEnd = (e: TouchEvent) => {
      for (const t of Array.from(e.changedTouches)) {
        if (t.identifier === mId.current) {
          mId.current = null;
          if (moveJoyRef.current) {
            moveJoyRef.current.x = moveJoyRef.current.y = 0;
          }
          setMk({ x: 0, y: 0 });
          setMBase(null);
        }
        if (t.identifier === lId.current) {
          lId.current = null;
          if (lookJoyRef.current) {
            lookJoyRef.current.x = lookJoyRef.current.y = 0;
          }
          setLk({ x: 0, y: 0 });
          setLBase(null);
        }
    ...
    >
      ...
      >
        ...
        {activities.map((a) => {
          if (a.type === "campfire")
            return (
              <Campfire key={a.id} x={a.x} z={a.z} interacted={a.interacted} />
            );
          if (a.type === "well") return <Well key={a.id} x={a.x} z={a.z} />;
          if (a.type === "chest")
            return (
              <Chest key={a.id} x={a.x} z={a.z} interacted={a.interacted} />
            );
          if (a.type === "signpost")
            return <Signpost key={a.id} x={a.x} z={a.z} />;
          return null;
        })}
